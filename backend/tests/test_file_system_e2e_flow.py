...
async def test_full_file_lifecycle():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        fr = await client.post("/api/files/folder", json={"name": f"e2e-folder-{uid}"}, headers=headers)
        folder_id = fr.json()["data"]["id"]
        try:
            resp = await client.post("/api/files/upload", files={"file": (f"e2e-{uid}.txt", b"e2e content")}, data={"folder_id": folder_id}, headers=headers)
