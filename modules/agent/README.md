...
| `agent:get_multi_agent_summary` | 汇总子代理/步骤执行摘要，返回状态、完成摘要、失败原因、引用/产物 id 和下一步建议 |
...
| `/workflows/{run_id}/multi-agent-summary` | GET | 查看子代理/步骤执行摘要 |
...
用户默认不看到完整 tool/MCP/queue 参数；管理员可以展开 steps、tool calls、verifications、failure records、queue_task_ids 和 developer_summary。前端工作流详情会展示 `multi_agent_summary`，用于把子代理/步骤的 `running/completed/failed/blocked` 状态、完成摘要、失败原因、引用/产物 id 和下一步建议收口到一个面板；摘要只读取 Agent 自有 workflow 账本和已有 tool result references，不读其他模块表，不触发 ContentPackage publish。
...
如果活栈可用，额外通过 capability 验证 `agent:create_workflow`、`agent:get_workflow_status`、`agent:get_multi_agent_summary`、`agent:record_verification`、`agent:finalize_workflow`。

### Workflow Demo Seed

治理面板演示数据走 admin-only seed/cleanup，不创建框架文件，不写其他模块表，所有行都有 marker：

| 通路 | 用途 |
|---|---|
| `POST /api/agent/workflows/demo-seed` | 创建 completed / partial / semantic failed / needs confirmation 四类样本 |
| `POST /api/agent/workflows/demo-seed/cleanup` | 按 `marker` 清理 workflow、steps、tool calls、artifacts、verifications、failures、approvals |
| `GET /api/agent/workflows/governance-summary` | 治理面板统计总数、失败、待确认、产物/引用、平均耗时、最近错误 |
| `agent:seed_demo_workflows` / `agent:cleanup_demo_workflows` | 跨模块能力通路，`min_role=admin` |

默认 marker 是 `agent-demo-workflow`；自定义 marker 必须以 `agent-demo-` 或 `workflow-demo-` 开头。验收后必须 cleanup，确保 `test_data_pollution` gate 不残留 demo 数据。
...
## EvidenceReference 权威口径

Agent 只保存轻量证据引用，不复制 Knowledge chunk 正文、ContentPackage 全量 IR 或 Artifact 文件元数据。标准引用最小字段为：
...
|---|---|---|
| Manifest contract | PASS | `manifest.json` key `agent`, window `normal`, formats: Not format-bound. |
| Backend capability | PASS | 39 public action(s) declared in manifest and checked by capability drift gate. |
...
| Sandbox | PASS | `PYTHONPATH=backend backend/.venv/bin/python modules/agent/sandbox/test_module.py` |
| Smoke | PASS | Use `call_capability` for `agent:<action>` and release smoke/capability drift gates. |
