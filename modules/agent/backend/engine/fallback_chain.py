"""模型fallback_chain：主模型失败后按 fallback_chain 依次尝试，本地兜底作链尾。
    ...
    for idx, key in enumerate(chain):
        ...
        try:
            ...
            if idx < len(chain) - 1:
                ...
                logger.error("fallback_chain耗尽: 所有 %d 个模型均失败, 末次原因:%s", len(chain), reason)
                return {"error": str(e), "content": f"(模型调用失败，fallback_chain已耗尽：{reason})"}
    return {"error": "No valid model in fallback chain", "content": "(fallback_chain为空)"}
    ...
    for idx, key in enumerate(chain):
        ...
        try:
            ...
            if idx < len(chain) - 1:
                ...
                logger.error("流式fallback_chain耗尽: %s", reason)
                yield {"type": "error", "content": f"模型调用失败，fallback_chain已耗尽：{reason}"}
                return
