...
class TestMatchWorkflow:
    def test_no_default_seeded_workflow_match(self):
        assert match_workflow("这个项目需要收口") is None
        assert match_workflow("数据库迁移需要执行") is None
        assert match_workflow("新建模块 user") is None
...
class TestApplyWorkflowInjection:
    ...
    def test_injection_into_first_system_message_only(self):
        ...
        assert messages[0]["content"] == "first sys"
