...
SYSTEM_BASE_PROMPT = (
    ...
    "为省 token 默认不在此罗列。做任何任务：先调用 skill_list 查看可用技能（display_name 是给用户看的中文名，name 是内部调用名）→ 用 skill_describe "
    "看目标技能的参数 → 用 skill_use 调用它。对用户展示技能列表时用中文 display_name，不要把英文内部 name 当作显示名。"
