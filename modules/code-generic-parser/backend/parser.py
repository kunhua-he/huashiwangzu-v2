"""本解析器：通用代码兜底（java/go/c/cpp/cs/rb/rs/sh/sql），按注释/关键字/大括号/空行粗切。

支持格式：.c, .cpp, .cs, .go, .java, .rb, .rs, .sh, .sql
...
MODULE_KEY = "code-generic-parser"
SUPPORTED_EXTS = {'sh', 'c', 'go', 'cpp', 'java', 'cs', 'sql', 'rs', 'rb'}
