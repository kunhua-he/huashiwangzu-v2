...
router = APIRouter(prefix="/api/code-js-parser", tags=["code-js-parser"])
...
@router.get("/health")
async def health():
    return ApiResponse(data={"module": "code-js-parser", "status": "ok"})
...
register_capability(
    "code-js-parser", "parse", _parse,
    description="Parse JS/TS files into semantic code blocks",
    brief="解析 JS/TS 代码",
...
register_capability(
    "code-js-parser", "reload_rules", _reload_rules,
