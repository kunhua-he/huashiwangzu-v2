...
Knowledge 前端入口按“入口布局 + 工作区状态组合 + API/types”拆分：

| 路径 | 作用 |
|---|---|
| `frontend/index.vue` | 桌面入口，保留左侧文件树、主区域布局、弹窗和样式 |
| `frontend/composables/useKnowledgeWorkspace.ts` | 文件树、进度握手、文档打开、分析轮询、搜索、导出和问 AI 的状态组合 |
| `frontend/types/index.ts` | 入口 props 与 Knowledge API/graph 类型 re-export |
| `frontend/api.ts` | 现有 API helper 与后端字段类型，保持对 `views/` 组件兼容 |

本次拆分不改变自动登记、补跑、进度轮询、搜索引用、导出或图谱行为。入口文件已降到 600 行以内；CSS 暂保留在入口，避免视觉回归。
