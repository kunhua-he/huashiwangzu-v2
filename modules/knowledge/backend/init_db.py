...
KB_TABLES = [
    ...
    "kb_source_file_manifest", "kb_validation_reports", "kb_artifact_lineage", "kb_terms",
...
_INDEX_STATEMENTS = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_kb_source_manifest_owner_root_status ON kb_source_file_manifest(owner_id, source_root, import_status)",
    "CREATE INDEX IF NOT EXISTS idx_kb_source_manifest_owner_ext_status ON kb_source_file_manifest(owner_id, extension, import_status)",
    "CREATE INDEX IF NOT EXISTS idx_kb_source_manifest_scan ON kb_source_file_manifest(owner_id, scan_id)",
    "CREATE INDEX IF NOT EXISTS idx_kb_source_manifest_task ON kb_source_file_manifest(import_task_id) WHERE import_task_id IS NOT NULL",
    "CREATE INDEX IF NOT EXISTS idx_kb_source_manifest_doc ON kb_source_file_manifest(document_id) WHERE document_id IS NOT NULL",
...
7. 对于只含图片技术分析信息的页面，subject 写为该图片的实际内容描述（如”产品实拍图”、”证书扫描件”），而非”该文件仅包含一页图片的技术分析信息”。""",
    },
    {
...
提取规则：
1. 实体类型：人名、品牌、产品、成分、功效、机构、证书、规格、检验标准、备案批号、术语、组织名、地名、事件、其他
2. 关系类型：属于、位于、产生、包含、引用、相关、领导、拥有、参与
3. 每条实体/关系必须基于原文证据
4. 提取出的实体名称要规范化（全称优先）
5. 禁止抽取以下类型词汇作为实体：图片元数据（RGB/像素/亮度/边缘密度/色彩模式）、文件格式名（JPEG/PNG/PDF）、技术参数（分辨率/DPI）、纯数字、单字符
...
如果无实体可抽，返回 {"entities": [], "relationships": []}。""",
    },
    {
        ...
        "content": """你是护肤品/化妆品企业资料库的文档内容融合专家。本知识库服务于华世王镞护肤品牌。

以下是对同一文档页的三轮独立采集结果，请交叉印证后输出融合内容。
...
4. 保持原文信息不丢失，禁止编造
5. 用中文输出
6. 以下内容不属于正文信息，必须从 fused_text 中排除：PDF工具名、文件系统路径、像素尺寸/分辨率/DPI、图片元数据（平均亮度、色彩空间、EXIF）、转换工具占位文本
