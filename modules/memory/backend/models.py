...
from sqlalchemy import Boolean, Float, Integer, String, Text, DateTime, BigInteger, func as sa_func
...
class AgentMemory(Base):
    ...
    confidence: Mapped[float] = mapped_column(Float, default=1.0, server_default=sa_func.text("1.0"), comment="置信度 0-1")
    recency_score: Mapped[float] = mapped_column(Float, default=1.0, server_default=sa_func.text("1.0"), comment="时效分，dream 衰减用")
    ...
    access_count: Mapped[int] = mapped_column(Integer, default=0, server_default=sa_func.text("0"), comment="被召回次数")
...
class AgentExperience(Base):
    ...
    success_weight: Mapped[int] = mapped_column(Integer, default=1, server_default=sa_func.text("1"), comment="成功权重")
    fail_count: Mapped[int] = mapped_column(Integer, default=0, server_default=sa_func.text("0"), comment="失败次数")
    ...
    active: Mapped[bool] = mapped_column(Boolean, default=True, server_default=sa_func.text("true"), comment="是否启用（dream 可停用低质经验）")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=sa_func.now(), default=lambda: datetime.now(timezone.utc))
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=sa_func.now(), default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))
