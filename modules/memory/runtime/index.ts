...
// ── Memory module-specific API ──────────────────────────────────────

const MEMORY_API_BASE = '/api/memory'

export const memory = {
  async save(params: { text: string; tags?: string; source?: string; conversation_id?: number }): Promise<{ id: number; status: string }> {
    return apiPost(`${MEMORY_API_BASE}/save`, params)
  },
  async recall(params: { query: string; limit?: number; expand_chain?: boolean }): Promise<MemoryItem[]> {
    return apiPost(`${MEMORY_API_BASE}/recall`, params)
  },
  async list(limit = 50, offset = 0): Promise<MemoryItem[]> {
    return apiGet(`${MEMORY_API_BASE}/list?limit=${limit}&offset=${offset}`)
  },
  async delete(id: number): Promise<{ id: number; status: string }> {
    return apiPost(`${MEMORY_API_BASE}/delete`, { id })
  },
  async fuse(params: { query: string; ids: number[] }): Promise<{ fused: string; source_ids: number[]; note: string }> {
    return apiPost(`${MEMORY_API_BASE}/fuse`, params)
  },
  async rethink(params: { id: number; text: string; tags?: string }): Promise<{ id: number; status: string; old_text: string }> {
    return apiPost(`${MEMORY_API_BASE}/rethink`, params)
  },
  async replace(params: { id: number; old_text: string; new_text: string }): Promise<{ id: number; status: string }> {
    return apiPost(`${MEMORY_API_BASE}/replace`, params)
  },
  async insert(params: { id: number; text: string }): Promise<{ id: number; status: string }> {
    return apiPost(`${MEMORY_API_BASE}/insert`, params)
  },
}
