...
from .services.generation_contract import (
    generate_bytes_for_format as _generate_bytes_for_format,
)
from .services.generation_contract import (
    normalize_convert_format as _normalize_convert_format,
)
from .services.generation_contract import (
...
from .services.generation_contract import (
    render_generated_bytes as _render_generated_bytes,
)
from .services.generation_contract import (
...
_FILE_OUTPUT_SCHEMA = {
    ...
    "required": ["file_id", "resource_refs"],
}
_FILE_CREATE_EXECUTION_CONTRACT = {
...
_ARTIFACT_OUTPUT_SCHEMA = {
    ...
    "required": ["artifact_id", "resource_refs"],
}
_ARTIFACT_CREATE_EXECUTION_CONTRACT = {
    ...
    "idempotency": "supported",
    "side_effect_level": "create",
    "output_reference_types": ["artifact", "file"],
    "parallel_safe": False,
}


def _file_resource_ref(result: dict) -> dict:
    file_id = int(result["file_id"])
    return {
        "type": "file",
        "id": file_id,
        "locator": f"/api/files/detail/{file_id}",
        "mime_type": str(result.get("mime_type") or ""),
        "display_name": str(result.get("name") or f"File {file_id}"),
        "access_scope": "user",
        "provenance": {"module": "office-gen"},
    }


def _artifact_resource_refs(result: dict) -> list[dict]:
    refs = [{
        "type": "artifact",
        "id": int(result["artifact_id"]),
        "display_name": str(result.get("name") or f"Artifact {result['artifact_id']}"),
        "access_scope": "user",
        "provenance": {"module": "office-gen"},
    }]
    if result.get("file_id") is not None:
        refs.append(_file_resource_ref(result))
    return refs
...
async def _save_file_bytes(
    ...
    response["resource_refs"] = [_file_resource_ref(response)]
    return response
...
register_capability(
    ...
    min_role="editor",
    execution_contract=_FILE_CREATE_EXECUTION_CONTRACT,
)
...
register_capability(
    ...
    retrieval={
        "aliases": ["文档转换", "格式转换", "转 PDF", "转换办公文件"],
        "when_to_use": "用户要求把已有办公文件转换为另一种格式时",
        "when_not_to_use": "用户要求从零创建文档内容时",
...
async def _cap_generate_to_artifact(params: dict, caller: str) -> dict:
    ...
    response["resource_refs"] = _artifact_resource_refs(response)
    return response
...
register_capability(
    ...
    min_role="editor",
    execution_contract=_ARTIFACT_CREATE_EXECUTION_CONTRACT,
)
