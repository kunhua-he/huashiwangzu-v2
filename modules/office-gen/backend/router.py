...
# ── Helper: persist generated bytes as a framework File record ─────────
...
async def _save_file_bytes(
    ...
    """Save generated bytes to the framework file system and return file metadata."""
    ...
    content_path = f"{md5_hash[:2]}/{md5_hash[2:4]}/{md5_hash}.{ext}"
    ...
    async with AsyncSessionLocal() as db:
        ...
        if existing_file and abs_content_path.exists():
            ...
            await db.flush()
            await db.execute(
                update(File).where(File.id == existing_file.id)
