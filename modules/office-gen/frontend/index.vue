<template>
  <div class="office-gen-container">
    ...
    <div class="og-grid">
      <div class="og-card" v-for="fmt in formats" :key="fmt.key" @click="generateFormat(fmt.key)">
    ...
    <div class="og-status">
      ...
      LibreOffice: {{ libreofficeOk ? 'Available' : 'Not detected (conversion limited)' }}
    </div>
  </div>
</template>
...
async function generateFormat(_fmt: string) {
  // This module is primarily backend-driven via Agent capability calls.
  // Direct UI generation will be added in a future iteration.
}
