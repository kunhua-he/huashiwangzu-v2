...
Parse PDF files into unified content blocks via the framework's cross-module capability system.

## Capability

- Module: `pdf-parser`
- Action: `parse`
- Input: `{"file_id": int}` — File ID in framework file storage
- Output: Unified content block skeleton

## Usage

```python
from app.services.module_registry import call_capability
result = await call_capability("pdf-parser", "parse", {"file_id": 123}, caller="user:42")
```

## Dependencies

- `pdfplumber` (already installed in the backend venv)

## Format Support

- `.pdf` — Text extraction via pdfplumber, table extraction, embedded image detection

## Architecture

Single-file module: `backend/router.py` contains both the FastAPI router and the inline parsing handler. No separate service model.
Unified output skeleton: `{ "file_id", "format", "blocks", "resources" }`.
