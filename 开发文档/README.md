...
| 开发工具台 | **`dev_toolkit/` 项目 MCP**（标准自动发现入口为仓库根 `.mcp.json`，Python stdio 入口为 `python3.14 dev_toolkit/server.py`），核心工具含 brief/probe/call_capability/routes/capabilities/db_schema/tail_log/run_test/lint/code_explore/code_node/code_impact/quick_fix_preview/quick_fix_patch + memory_search/write/recent + smoke。**接手先连它**，详见 `dev_toolkit/README.md` |
...
脚本会检测 Git、Python 3.14、Node/npm、PostgreSQL/psql 等环境；缺系统依赖时 macOS 优先用 Homebrew，Windows 优先用 winget，其次 Chocolatey。脚本开始会提示配置代理，建议 Agent 帮别人安装时先填代理地址（如 `http://127.0.0.1:7890`），否则 Git、pip、npm、Homebrew/winget 下载容易慢或失败。
...
默认数据库参数：`127.0.0.1:5432`、用户 `postgres`、库名 `huashiwangzu_v2`。脚本会交互输入 PostgreSQL 密码和 admin/editor/viewer 种子用户默认密码；种子密码只作为本次环境变量传入，不写入仓库。初始化内容包括创建数据库、安装 `vector`/`pg_trgm` 扩展、创建框架表、插入种子数据、同步模块 manifest，并调用各模块自己的幂等 `init_db.py` 初始化入口。

常用选项：`--skip-system-install` 只检测不安装系统依赖，`--skip-frontend-build` 跳过前端构建，`--skip-start` 初始化后不启动后端。失败时先看终端输出和 `backend/logs/backend.log`。
