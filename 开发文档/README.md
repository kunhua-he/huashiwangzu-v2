...
| 模型网关 | `backend/app/gateway/` 四层治理：provider/variant/policy/template（`models.json`），统一 retry budget + health/budget 路由；调用入口 `gateway.service.chat(messages, profile_key=, tools=, budget=)`，**kwarg 是 `profile_key` 不是 model_profile** |
| 工作流/资源平台 | run ledger 三表 `framework_workflow_{definitions,runs,step_records}`，只读查询路由 `/api/workflow/*`；10 类资源对象统一表达（prompt/knowledge_source/document_ir/artifact/...）。当前为长期骨架，扩张挂点已留 |
| 文档 IR | parser 统一中间表达：manifest/blocks/resources/_trace，多格式归一 |
...
| 模块 | 作用 |
|---|---|
| `agent` | AI 助手：对话、工具发现、上下文引擎、记忆接入、子 Agent、治理面板。运行时已分层（ConversationRuntime/ToolLoopRuntime/StreamEmitter）；理解增强环（4 角色产 understanding_packet）；后台 Review Fork 受限复盘产 proposal（走审批门）；skill_manage 技能资产治理 |
| `memory` | 事实记忆、语义召回、记忆链、经验库、dream 自优化。selector/fencing/frozen snapshot；嵌入入库走 `CAST(:embedding AS vector)`（勿用 `::vector` 拼接，会与命名参数冲突致静默退化） |
| `knowledge` | 知识库：文件登记、解析、分块、检索、页级融合、实体图谱、治理候选。已升级为**证据控制面**：search 返 evidence_meta+citation+_trace，新增 evidence_search/get_evidence_detail；入库前 MD5 内容级去重 |
